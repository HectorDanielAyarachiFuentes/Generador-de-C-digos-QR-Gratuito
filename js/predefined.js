...
const predefinedQRs = [
    ...
    },
    {
        ...
        color: "#FFB300",
        background: "#FFFFFF",
        description: "Ámbar cálido y acogedor"
    },
    {
        id: 9,
    ...
    },
    {
        ...
        color: "#757575",
        background: "#FFFFFF",
        description: "Gris versátil y equilibrado"
    },
...
function loadPredefinedQRs() {
    ...
    // Actualizar las categorías en los botones
...
function updateCategoryButtons(buttons) {
    // Obtener categorías únicas de los estilos predefinidos
    const categories = [...new Set(predefinedQRs.map(qr => qr.category))];
    
    // Actualizar texto y atributos de los botones existentes
    buttons.forEach(button => {
        ...
        // Actualizar o eliminar los demás botones
        if (categories.includes(category)) {
            // Actualizar el texto del botón según la categoría
            switch (category) {
                case 'colores': button.textContent = 'Colores Sólidos'; break;
            ...
            // Si la categoría no existe en los nuevos datos, cambiar el atributo
            // a una categoría existente o a 'all'
            if (categories.length > 0) {
                button.setAttribute('data-category', categories[0]);
                button.textContent = categories[0].charAt(0).toUpperCase() + categories[0].slice(1);
            } else {
    ...
    // Crear botones para categorías nuevas si es necesario
    const existingCategories = Array.from(buttons).map(btn => btn.getAttribute('data-category'));
    const newCategories = categories.filter(cat => !existingCategories.includes(cat));
    
    if (newCategories.length > 0 && buttons.length > 0) {
        const container = buttons[0].parentNode;
        ...
        newCategories.forEach(category => {
            ...
            switch (category) {
                case 'colores': newButton.textContent = 'Colores Sólidos'; break;
                case 'fondos': newButton.textContent = 'Fondos de Color'; break;
                case 'elegantes': newButton.textContent = 'Elegantes'; break;
                case 'corporativos': newButton.textContent = 'Corporativos'; break;
                case 'vibrantes': newButton.textContent = 'Vibrantes'; break;
                case 'pasteles': newButton.textContent = 'Pasteles'; break;
                case 'monocromaticos': newButton.textContent = 'Monocromáticos'; break;
                default: newButton.textContent = category.charAt(0).toUpperCase() + category.slice(1);
            }
